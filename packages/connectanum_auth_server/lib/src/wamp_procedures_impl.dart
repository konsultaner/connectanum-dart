...
class AuthServerProcedureBinding {
  ...
  Future<void> _handleHello(LazyInvocationPayload invocation) async {
    try {
      ...
      if (tokenFailure != null || _closed) {
        invocation.respondWith(
          argumentsKeywords: _failurePayload(tokenFailure ?? _closedFailure),
        );
        return;
      }
      ...
      final response = await _server._onHello(request, owner: this);
      invocation.respondWith(
  ...
  Future<void> _handleAuthenticate(LazyInvocationPayload invocation) async {
    try {
      ...
      final pending = _server._owned(transactionId, this);
      if (pending == null) {
        invocation.respondWith(
      ...
      final response = await _server._onAuthenticate(request, owner: this);
      invocation.respondWith(
  ...
  Future<void> _handleAbort(LazyInvocationPayload invocation) async {
    try {
      ...
      final transactionId = _requiredString(payload, 'transactionId');
      _optionalString(payload, 'reason');
      final pending = _server._owned(transactionId, this);
      if (pending != null) {
        _server._finish(pending, _abortedFailure);
      }
      invocation.respondWith(
