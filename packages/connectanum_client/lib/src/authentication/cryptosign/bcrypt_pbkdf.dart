...
import 'package:connectanum_client/src/authentication/cryptosign/bcrypt.dart';
...
class BcryptPbkdf {
  static void pbkdf(
    ...
    var blockB = Uint8List(4);
    Uint8List out;
    Uint8List tmp;

    for (var block = 1; block <= nblocks; block++) {
      // Block count is in big endian
      blockB[0] = ((block >> 24) & 0xFF);
      blockB[1] = ((block >> 16) & 0xFF);
      blockB[2] = ((block >> 8) & 0xFF);
      blockB[3] = (block & 0xFF);

      sha512.reset();
      sha512.update(salt, 0, salt.length);
      sha512.update(blockB, 0, blockB.length);
