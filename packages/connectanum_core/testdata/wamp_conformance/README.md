...
- upstream suite is still open, so this repo vendors the currently useful
  single-message subset plus one router-level multi-session vector for CI and
  regression coverage
...
- upstream sequence/router-level vectors beyond the one multi-session publish
  exclusion case that are not fully checked in or are still evolving in the
  open PR
