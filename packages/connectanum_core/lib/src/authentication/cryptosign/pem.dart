...
class Pem {
  ...
  static Uint8List loadPrivateKeyFromOpenSSHPem(
    ...
    var binaryContent = base64.decode(
      pemFileContent.replaceAll(RegExp(r'[\n\r]'), ''),
    );
    ...
    if (String.fromCharCodes(header) == 'openssh-key-v1') {
      ...
      readerIndex += 4;
      var cypherName = _readOpenSshKeyString(
        binaryContent,
        readerIndex,
        cypherNameLength,
      );
      readerIndex += cypherNameLength;
      var keyDerivationFunctionNameLength = _readOpenSshKeyUInt32(
        binaryContent,
        readerIndex,
      );
      readerIndex += 4;
      var keyDerivationFunctionName = _readOpenSshKeyString(
        binaryContent,
        readerIndex,
        keyDerivationFunctionNameLength,
      );
      readerIndex += keyDerivationFunctionNameLength;
      var keyDerivationFunctionOptionsLength = _readOpenSshKeyUInt32(
        binaryContent,
        readerIndex,
      );
      readerIndex += 4;
      ...
      if (keyDerivationFunctionName == 'none') {
        ...
        var pbkdfSaltLength = _readOpenSshKeyUInt32(
        ...
        final cipherText = binaryContent.sublist(
          privateKeyIndex,
          privateKeyIndex + privateKeyLength,
        );

        var offset = 0;
        while (offset < privateKeyLength) {
          offset += cypher.processBlock(
            cipherText,
            offset,
            paddedPlainText,
            offset,
  ...
  static Uint8List _readOpenSshPrivateKeySeed(
    ...
    readerIndex += 4;
    var keyType = _readOpenSshKeyString(
      binaryContent,
      readerIndex,
      keyTypeLength,
    );
