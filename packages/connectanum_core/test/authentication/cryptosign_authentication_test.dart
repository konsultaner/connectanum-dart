...
void main() {
  group('CRYPTOSIGN', () {
    ...
    test('message handling', () async {
      ...
      for (var vector in testVectors) {
        ...
        expect(
          ...
          equals(
            authMethod.privateKey.publicKey.encode(Base16Encoder.instance),
          ),
        );
    ...
    test('load putty private key file', () async {
      final unencryptedPpkKey = Ppk.loadPrivateKeyFromPpk(
      ...
      final unencryptedOpenSshPpkKey = Ppk.loadPrivateKeyFromPpk(
    ...
    test('constructors', () async {
      ...
      expect(
        ppkKey.privateKey.sublist(0, 32).toString(),
      ...
      expect(
        opensshKey.privateKey.sublist(0, 32).toString(),
        equals(puttySeed.toString()),
      );

      var base64Key = CryptosignAuthentication.fromBase64(
        base64.encode(puttySeed),
      );
      expect(
        base64Key.privateKey.sublist(0, 32).toString(),
        equals(puttySeed.toString()),
      );
    });
    test('on challenge event', () async {
      final authMethod = CryptosignAuthentication.fromBase64(
        base64.encode(puttySeed),
      );
      ...
      var extra = Extra(
        challenge: testVectors[0]['challenge'],
        channelBinding: null,
      );
