...
- `native/bench/scenarios/wamp_smoke.toml` – WAMP-only RawSocket/WebSocket comparison scenario with representative JSON/MsgPack/CBOR coverage.
- `native/bench/scenarios/wamp_serializer_matrix.toml` – larger-payload RawSocket/WebSocket RPC serializer sweep (`json`, `msgpack`, `cbor`).
