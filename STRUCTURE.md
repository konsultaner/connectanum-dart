...
| Benchmarks & docs | Harness, auth docs, example gallery. | 🧭 Dart HTTP bench runner + Rust orchestrator are in place; the bench path now ships transformed Prometheus artifacts, sustained-transfer scenarios that reuse hot HTTP/2/HTTP/3 sessions, router-worker sweep support for sanity checks, and native-runtime thread sweeps so HTTP throughput can be graphed by both `router_workers` and `native_runtime_threads`. The Dart boss no longer adds a fixed busy-loop sleep or per-request debug logging on the HTTP hot path, so the sweep data better reflects real transport limits. |
