...
- 2026-06-14: Extended live fast-smoke coverage for the checked-in public
  router-hosted MCP client example to cover both public and bearer-protected
  router-provided MCP endpoints. `run_router_hosted_mcp_example_smoke` still
  first dry-runs `packages/connectanum_mcp/example/router_hosted_client.dart`
  with representative tool, resource, prompt, and pub/sub options, then
  ...
  long-lived server, waits for both the public and bearer-protected endpoint
  lines, invokes the public client live against the public endpoint with direct
  JSON tool/resource/prompt and pub/sub options, derives the example auth URL,
  and then invokes the same public client against the secure endpoint with the
  ticket auth-grant options. The helper still shuts the server down through
  bounded cleanup with a SIGKILL fallback plus server-log output on startup
  failure or timeout. This proves the consumer-facing example can use both
  public and authenticated router-hosted MCP through
  `package:connectanum_mcp/connectanum_mcp_io.dart` without private project
  imports or a hand-maintained endpoint.
  `tool/test_mcp_consumer_package_boundary.py` now guards both the dry-run and
  live-smoke wiring so the fast path keeps exercising the public package
  boundary and the auth-grant command line. Baseline `bin/test-fast` passed
  ...
  `Authenticated router-hosted MCP client live smoke completed.` fast-smoke
  ...
  consumer-package smokes, the router-hosted MCP live public and authenticated
  public-client examples, the installed CLI token-only protected WAMP
  ...
  clean at the previous pushed checkpoint `a2b55c8` until this local checkpoint
  is pushed and hosted checks complete: GitHub `master` CI `27480091176` passed
  with `Fast Checks` and `Full Verify` clean, and GitHub `add-router` CI
  `27480088243` also passed at `a2b55c8`. The strict deployment-chain audit
  exited successfully on 2026-06-13 with clean latest CI logs, relevant Dart
  package publish dry-run `27477985337` at `b9556f5`, relevant Native Artifacts
  ...
  ready. The audit accepted the older publish, native, router image, and WAMP
  ...
  numeric RC tag points at `a2b55c8`, the existing `v0.1.0-rc.1` tag still
  points at stale commit `47bbf9c`, no GitHub prerelease or router image RC tag
  is selected for `a2b55c8`, the audit suggests `v0.1.0-rc.2` only after
