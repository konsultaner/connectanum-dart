...
- 2026-07-10: Hardened HTTP auth grant response metadata parsing in
  ...
  `authprovider` fields that are empty or contain whitespace/control
  characters before returning bearer grants to consumer applications. The
  focused malformed-grant regression failed before the parser change, then
...
- 2026-07-10: Hardened typed MCP WAMP helper result parsing in
  ...
  `29058796269`, Dart Package Publish Dry Run `29058796268`, WAMP Profile
  Benchmarks `29058796315`, and the deployment-chain audit with required
  latest CI plus Dart package dry-run evidence passed for `713bc90`; the audit
...
- 2026-07-07: Added protected router-hosted MCP direct JSON WAMP helper
  ...
  rejects missing and invalid bearer direct JSON calls without issuing
  `mcp-session-id`, then verifies bearer-backed direct JSON `POST` access to
  `tools/list`, `connectanum.api.list`, WAMP session and registration meta
  helpers, and `connectanum.pubsub` subscribe/publish/poll/unsubscribe on the
  secure route. It asserts direct helper metadata is scoped to the authenticated
  member principal and that direct JSON responses remain lifecycle-free instead
  of becoming Streamable HTTP sessions. Baseline `bin/test-fast` passed before
...
- 2026-07-07: Added protected router-hosted MCP auth/session isolation
  ...
  `python3 tool/check_public_artifact_references.py`, `git diff --check`, and
  full local `bin/verify` passed after the change on 2026-07-07. Hosted
  evidence after push: commit `c8a2798` was pushed to GitLab `origin`
  `add-router`, GitHub `add-router`, and GitHub `master`; GitHub `master` CI
  `28843544922`, Dart Package Publish Dry Run `28843544936`, WAMP Profile
  Benchmarks `28843544935`, and the strict deployment-chain audit passed.
  GitHub `add-router` CI `28843541469`, Dart Package Publish Dry Run
...
- 2026-07-06: Hardened the public router-hosted `connectanum_mcp` client
  ...
  same-topic Streamable subscription, subscribes through public direct helpers,
  sends notification-only events through both direct event and direct method
  helpers, polls through direct helpers, unsubscribes, and asserts the active
  Streamable `sessionId` and `lastEventId` did not change. The ordering is
  intentional because WAMP same-session same-topic subscribe/unsubscribe
  semantics can share and then remove the active subscription. The public live
  smoke now requires `activeDirectJson.sessionUnchanged` for public,
  pub/sub-only, authenticated, bearer-token, authenticated JSON-response, and
  bearer-token JSON-response runs, and the package-boundary test pins the
  public example labels, helper IDs, source gate, and summary fragment.
  Baseline `bin/test-fast` passed before the change on 2026-07-06. Focused
  `bash -n bin/common.sh`, `git diff --check`, `dart analyze
  packages/connectanum_mcp`, full `python3 -m unittest
  tool.test_mcp_consumer_package_boundary -v`,
  `python3 tool/check_public_artifact_references.py`, and direct
  `run_public_router_hosted_mcp_client_live_smoke` passed after the change.
  Full local `bin/verify` passed on 2026-07-06, including formatting,
...
- 2026-07-06: Hardened the router CLI consumer package smoke so consumer
  ...
  and the Chrome/Dart2Wasm browser WebSocket smoke. Hosted evidence after
  push: commit `b7275e2` was pushed to GitLab `origin` `add-router`, GitHub
  `add-router`, and GitHub `master`; GitHub `master` CI `28800435799` and
  GitHub `add-router` CI `28800429286` passed with Fast Checks and Full Verify
  green; Dart Package Publish Dry Run remains clean and relevant from
  `a1ebb68`; WAMP Profile Benchmarks remain clean and relevant from
  `a1ebb68`; Router Image dry-run `28787623457` remains clean and relevant
  ...
  GitHub `master` at `b7275e2` with CI log scan clean and clean/relevant Dart
...
- 2026-07-06: Hardened the generated router CLI consumer package smoke so
  ...
  and internal metrics realm. The smoke now parses the generated OpenMetrics
  listener, checks GET and HEAD on `/healthz`, `/health`, and `/metrics`, and
  verifies the metrics payload includes the
  `connectanum_router_drain_in_progress` metric before running the existing
  MCP/auth/session matrix. The package-boundary test pins the metrics internal
  realm config, listener log parsing, all GET and HEAD probes, and the updated
  human-readable smoke evidence so the readiness smoke cannot silently regress
  to a sidecar/private-project health check. Baseline `bin/test-fast` passed
  before the change on 2026-07-06. Focused `bash -n bin/common.sh`,
...
- 2026-07-06: Extended the generated router CLI consumer package smoke so
  ...
  router integration, and the Chrome/Dart2Wasm browser WebSocket smoke. Hosted
  evidence after push: commit `52f1ae3` was pushed to GitLab `origin`
  `add-router`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `28768310605` and GitHub `add-router` CI `28768307878` passed with Fast
  Checks and Full Verify green. The strict deployment-chain audit passed for
  GitHub `master` at `52f1ae3` with CI log scan clean and clean/relevant Dart
  package, native release, router image, and WAMP profile evidence; RC
...
- 2026-07-06: Extended the generated router CLI consumer package smoke so
  ...
  sessions, polls with an unknown event id scoped to the current session,
  expects `McpStreamableHttpException` with HTTP 400 and an error body that
  names `Last-Event-ID`, then verifies the session id and last event id remain
  unchanged. The smoke summary now reports `streamableInvalidLastEventId: true`
  for public, protected, and token-only routes, the shell summary gate requires
  those keys, and `tool/test_mcp_consumer_package_boundary.py` pins the helper,
  route labels, summary keys, and human-readable smoke evidence. Baseline
  `bin/test-fast` passed before the change on 2026-07-06. Focused
  `bash -n bin/common.sh`,
...
- 2026-07-06: Hardened the bench CLI consumer package smoke so local WAMP
  ...
  router integration, and the Chrome/Dart2Wasm browser WebSocket smoke. Hosted
  evidence after push: commit `cbae40d` was pushed to GitLab `origin`
  `add-router`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `28763944019` and GitHub `add-router` CI `28763943941` passed with Fast
  Checks and Full Verify green. The strict deployment-chain audit passed for
  GitHub `master` at `cbae40d` with CI log scan clean and clean/relevant Dart
  package, native release, router image, and WAMP profile evidence; RC
  readiness is blocked only on selecting/approving the numeric RC
  tag/prerelease/router-image tag plus the deferred pub.dev package
  ownership/order track.
...
- 2026-07-06: Extended public router-hosted MCP live smoke coverage for direct
  ...
  `tools/list` POST after a Streamable session is active, includes
  `Accept: application/json`, the current `MCP-Protocol-Version`, and a
  syntactically valid but unknown `MCP-Session-Id`, and asserts the router
  returns a normal JSON-RPC result without echoing a session header or mutating
  the typed Streamable session id/SSE cursor. The live smoke summary now
  reports `directJsonStaleSessionId: {ignored: true, sessionUnchanged: true}`,
  `bin/common.sh` requires that field across public, pub/sub-only, auth-grant,
  bearer, and JSON-response variants, and the package-boundary test pins the
  generated helper, trace header, failure messages, and summary key. Baseline
  `bin/test-fast` passed before the change on 2026-07-06. Focused
  ...
  2026-07-06, including formatting, Rust/FFI tests, MCP package tests,
  consumer package smokes, live WAMP benchmark integration, router-hosted MCP
...
- 2026-07-05: Fixed router-hosted MCP malformed session-header validation for
  ...
  WebSocket smoke. Hosted evidence after push: commit `50b2458` was pushed to
  GitLab `origin` `add-router`, GitHub `add-router`, and GitHub `master`.
  GitHub `master` CI `28757352754` and GitHub `add-router` CI `28757352718`
  passed with Fast Checks and Full Verify green. Dart Package Publish Dry Run
  passed on GitHub `master` (`28757352735`) and GitHub `add-router`
  (`28757352719`), WAMP Profile Benchmarks passed on GitHub `master`
  (`28757352728`) and GitHub `add-router` (`28757352711`), and Router Image
  dry-run `28757835462` passed on GitHub `master` at `50b2458` with GHCR login
  skipped. The strict deployment-chain audit passed for GitHub `master` at
  `50b2458`; RC readiness is blocked only on selecting/approving the numeric
  RC tag/prerelease/router-image tag plus the deferred pub.dev package
  ownership/order track.
- 2026-07-05: Extended router CLI consumer package smoke coverage for
  ...
  session id and SSE resume cursor before `deleteSession()`, creates a fresh
  authenticated `McpStreamableHttpClient`, replays the stale session state
  before each matrix operation, requires `404` for direct batches, WAMP
  meta/pubsub batches, notification-only `initialized`, `tools/list`,
  `tools/call`, `resources/list`, `resources/read`,
  `resources/templates/list`, `prompts/list`, `prompts/get`, `poll()`, and
  `deleteSession()`, and asserts the client clears stale session/cursor state
  each time. The smoke summary now exports `deletedSessionRejected: true` and
  `deletedSessionMatrix: true` for the secure path and prints an explicit
  matrix replay rejection line; the boundary test pins the generated helper,
  operation IDs, summary flag, and evidence line. Baseline `bin/test-fast`
...
- 2026-07-05: Expanded router-native OpenMetrics/health internal-route
  ...
  `/health` alias, preserves `cache-control: no-store`, and suppresses the
  response body for HEAD health requests while retaining text headers. Baseline
  `bin/test-fast` passed before the change on 2026-07-05. Focused
...
- 2026-07-05: Implemented file-backed HTTP response streaming for WAMP-backed
  ...
  browser WebSocket smoke. Hosted evidence after push: commit `9bde9da` was
  pushed to GitLab `origin` `add-router`, GitHub `add-router`, and GitHub
  `master`. GitHub `add-router` CI `28744999936`, Dart Package Publish Dry
  Run `28744999932`, and WAMP Profile Benchmarks `28744999962` passed at
  `9bde9da`; GitHub `master` CI `28745005169`, Dart Package Publish Dry Run
  `28745005161`, WAMP Profile Benchmarks `28745005165`, and Router Image
  dry-run `28745447883` also passed at `9bde9da`. Router Image dry-run preview
  metadata was `sha-9bde9da2c2c7`, with GHCR login/push skipped. Native
  Artifacts dry-run `28741330334` remains clean and relevant from `d539a34`
  because this checkpoint did not touch native-release-sensitive paths. The
...
- 2026-07-04: Added a neutral benchmark CLI consumer package smoke for local
  ...
  `connectanum_bench` through public package metadata and neutral path
  overrides, uses hook user-defines to avoid native rebuilds during executable
  ...
  private project assumptions. `bin/test-fast` and `bin/test-all` run the
  smoke after the bench VM tests, and
  ...
  `--config` / `--native-lib` help output. Baseline `bin/test-fast` passed
  before the change on 2026-07-04, including router-hosted MCP live smokes,
  the neutral MCP consumer package smoke, the package-executable router CLI
  consumer smoke, live WAMP benchmark integration, and router fast tests.
  Focused `bash -n bin/common.sh bin/test-fast bin/test-all bin/verify`,
  `python3 -m py_compile tool/test_mcp_consumer_package_boundary.py tool/test_verification_scripts.py`,
  `python3 tool/test_mcp_consumer_package_boundary.py`,
  ...
  benchmark integration, the neutral benchmark CLI consumer package smoke,
...
- 2026-07-04: Hardened the router CLI consumer package smoke to prove the
  ...
  checkout command, and no longer runs `dart pub global activate` for the
  router executable. The generated Dart consumer package still uses its
  isolated `PUB_CACHE` for dependency resolution and analysis. This proves a
  consumer application or agent harness can put the checkout `bin/` directory
  on `PATH` and exercise router-hosted MCP without relying on a global pub
  cache or private project assumptions. The MCP consumer package boundary test
  now guards the checkout command lookup, launch path, final smoke summary, and
  absence of pub-global activation for the router executable. Baseline
  `bin/test-fast` passed before the change on 2026-07-04. Focused
  `bash -n bin/common.sh`, focused `python3 -m py_compile
  tool/test_mcp_consumer_package_boundary.py`, focused
...
- 2026-07-04: Added an executable-compatible source-checkout router alias for
  ...
  the Chrome/Dart2Wasm browser WebSocket smoke. Commit `ce9366d`
  (`tooling: add connectanum_router checkout alias`) was pushed to GitLab
  `origin` `add-router`, GitHub `add-router`, and GitHub `master`. GitHub
  `add-router` CI `28707268764`, Dart Package Publish Dry Run `28707268762`,
  and WAMP Profile Benchmarks `28707268766` passed and covered the checked-out
  head. GitHub `master` CI `28707270451`, Dart Package Publish Dry Run
  `28707270442`, and WAMP Profile Benchmarks `28707270450` also passed and
  covered the checked-out head. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --show-wamp-profile-benchmarks --require-clean-wamp-profile-benchmarks --strict`
  exited successfully on 2026-07-04 with protected `master` branch status
  checks, workflow visibility, router image package visibility, latest GitHub
  `master` CI evidence, latest Dart package dry-run evidence, and latest WAMP
  profile benchmark evidence clean at `ce9366d`.
...
- 2026-06-20: Hardened repository verification scripts so root smoke helpers
  ...
  stall before the validation-only example client reached its expected argument
  error; focused direct execution with analytics suppression exited with the
  expected usage error. `tool/test_verification_scripts.py` now asserts the
  default environment after sourcing `bin/common.sh`. After the change, focused
  `bash -n bin/common.sh bin/test-fast bin/test-all`, focused
  `python3 tool/test_verification_scripts.py`, and focused
...
- 2026-06-20: Tightened the generated router CLI Dart MCP consumer smoke so
  ...
  downstream consumer on `/mcp/secure-json-post`. `bin/common.sh` now configures
  page-size-one protected resources, resource templates, and prompts plus
  second-page entries, follows `nextCursor` through direct JSON and Streamable
  `resources/list`, `resources/templates/list`, and `prompts/list`, verifies the
  second page entries and terminal null cursor, and keeps asserting the helper
  calls do not mutate the JSON-response route's Streamable session id or SSE
  cursor. The human-readable router CLI smoke summary now distinguishes direct
  JSON and Streamable pagination coverage.
  `tool/test_mcp_consumer_package_boundary.py` guards the new generated request
  IDs, cursor failure messages, and summary fragment. Baseline `bin/test-fast`
  passed before the change on 2026-06-20. Focused `bash -n bin/common.sh`,
  focused `python3 tool/test_mcp_consumer_package_boundary.py`, and focused
  `bash -lc 'set -euo pipefail; source bin/common.sh; cd_repo_root; run_router_cli_consumer_package_smoke'`
  passed on 2026-06-20, with the installed router CLI consumer smoke exercising
  the new generated Dart protected JSON-response active direct JSON and
  Streamable pagination checks and emitting the updated smoke summary. Full
  local `bin/verify` passed on 2026-06-20, including formatting, Rust/FFI tests,
  Python/tool tests, MCP package tests, consumer package smokes, router-hosted
  MCP example smokes, the installed router CLI consumer smoke with the new
  protected JSON-response pagination checks, full router tests, zero-copy router
  tests, and the Chrome/Dart2Wasm browser WebSocket smoke. Commit `0f29c0d`
  (`test: cover json response pagination smoke`) was pushed to GitLab `origin`,
  GitHub `add-router`, and GitHub `master`. GitHub `master` CI `27877228100`
  and GitHub `add-router` CI `27877227462` passed with `Fast Checks` and
  `Full Verify` clean. No new Dart Package Publish Dry Run or WAMP Profile
  Benchmarks were required because no publish-sensitive or benchmark-sensitive
  package inputs changed since `7202eaf`; the latest GitHub `master` Dart
  Package Publish Dry Run `27853666798` remains clean and relevant at
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `0f29c0d`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the generated router CLI Dart MCP consumer smoke so
  protected active JSON-response direct JSON resource readiness includes
  single-request `resources/list` and `resources/read` helper calls while a
  `/mcp/secure-json-post` Streamable session is already open. `bin/common.sh`
  now lists protected resources, reads `cli://mcp/secure/context`, verifies
  the protected resource content, and keeps asserting direct helpers plus
  batches do not mutate the JSON-response route's Streamable session id or SSE
  cursor. The human-readable router CLI smoke summary now distinguishes direct
  JSON resource list/read coverage.
  `tool/test_mcp_consumer_package_boundary.py` guards the new generated
  request IDs, failure messages, and summary fragment. Baseline
  `bin/test-fast` passed before the change on 2026-06-20. Focused
  ...
  passed on 2026-06-20, with the installed router CLI consumer smoke exercising
  the new generated Dart protected JSON-response active direct JSON
  resource-list/read checks and emitting the updated smoke summary. Full local
  `bin/verify` passed on 2026-06-20, including formatting, Rust/FFI tests,
  Python/tool tests, MCP package tests, consumer package smokes, router-hosted
  MCP example smokes, the installed router CLI consumer smoke with the new
  protected JSON-response active direct JSON resource list/read checks, full
  router tests, zero-copy router tests, and the Chrome/Dart2Wasm browser
  WebSocket smoke. Commit `d74f33e`
  (`test: cover active json response direct resources`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27874794408` and GitHub `add-router` CI `27874794496` passed with
  `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry Run
  or WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest
  GitHub `master` Dart Package Publish Dry Run `27853666798` remains clean and
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `d74f33e`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the generated router CLI Dart MCP consumer smoke so
  ...
  zero-copy router tests, and the Chrome/Dart2Wasm browser WebSocket smoke.
  Commit `d152644` (`test: cover json response streamable resource read`) was
  pushed to GitLab `origin`, GitHub `add-router`, and GitHub `master`. GitHub
  `master` CI `27873173083` and GitHub `add-router` CI `27873166905` passed
  with `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry
  Run or WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest
  GitHub `master` Dart Package Publish Dry Run `27853666798` remains clean and
  relevant at `7202eaf`. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `d152644`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the generated router CLI Dart MCP consumer smoke so
  ...
  lists protected resources, lists protected resource templates, lists
  protected prompts, and fetches a protected prompt while active Streamable
  sessions are already exercising WAMP metadata and pub/sub. The JSON-response
  route asserts the helper calls keep that route response-only for SSE state;
  the non-JSON-response route asserts the session id stays stable and the SSE
  cursor remains valid for that session.
  `tool/test_mcp_consumer_package_boundary.py` guards the new generated request
  IDs, prompt substitutions, failure messages, SSE-state assertion, and
  human-readable smoke summary fragments. Baseline `bin/test-fast` passed
  before the change on 2026-06-20. Focused `bash -n bin/common.sh`, focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_router_cli_consumer_smoke_exercises_raw_json_mcp_surface`,
  focused `git diff --check`, focused
  `python3 tool/check_public_artifact_references.py`, and focused
  `bash -lc 'source bin/common.sh; ensure_native_lib_env || true; run_router_cli_consumer_package_smoke'`
  passed on 2026-06-20, with the installed router CLI consumer smoke exercising
  the new generated Dart protected Streamable resources, resource templates,
  and prompts over both protected routes. Full local `bin/verify` passed on
  2026-06-20, including formatting, Rust/FFI tests, Python/tool tests, MCP
...
- 2026-06-20: Tightened the generated router CLI Dart MCP consumer smoke so
  ...
  WebSocket smoke. Commit `e1e0fe7`
  (`test: cover protected mcp procedure metadata`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27869908900` and GitHub `add-router` CI `27869906738` passed with
  `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry Run
  or WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest
  GitHub `master` Dart Package Publish Dry Run `27853666798` remains clean and
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `e1e0fe7`, and latest relevant Dart package dry-run evidence
  clean.
...
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  resource result, method-not-found error isolation, and WAMP topic API
  metadata, and asserts the direct batch leaves `sessionId`/`lastEventId`
  unchanged. The already-exercised active protected JSON-response Streamable
  batch is now guarded as explicit consumer evidence too. The emitted
  `routerCliConsumerSummary` now includes `jsonResponse.active.batch: true`,
  and the human-readable smoke summary names protected JSON-response direct JSON
  batch isolation plus Streamable batch/session-delete coverage.
  `tool/test_mcp_consumer_package_boundary.py` guards the new direct batch
  request IDs, existing Streamable batch IDs, failure messages, summary
  fragment, and summary text. Baseline `bin/test-fast` passed before the change
  on 2026-06-20. Focused `bash -n bin/common.sh`, focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_router_cli_consumer_smoke_exercises_raw_json_mcp_surface`,
  focused `git diff --check`, and focused
  `bash -lc 'source bin/common.sh; ensure_native_lib_env || true; run_router_cli_consumer_package_smoke'`
  passed on 2026-06-20, with the installed router CLI consumer smoke emitting
  `jsonResponse.active.batch: true`. Full local `bin/verify` passed on
  2026-06-20, including formatting, Rust/FFI tests, Python/tool tests, MCP
  package tests, generated consumer-package smokes, the router-hosted MCP live
  public, pub/sub-only, authenticated, bearer, and JSON-response examples, the
...
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  JSON and Streamable HTTP subscription/publish/poll/unsubscribe flows for the
  protected JSON-response endpoint; it now promotes that coverage into
  `routerCliConsumerSummary` with `jsonResponse.active.pubsub: true`, and the
  human-readable smoke summary names protected JSON-response direct JSON pub/sub
  isolation plus Streamable pub/sub/session-delete coverage.
  `tool/test_mcp_consumer_package_boundary.py` now guards the direct JSON and
  Streamable subscription/publish/poll/unsubscribe request IDs, failure
  messages, summary fragment, and summary text so future generated-smoke
  rewrites preserve this downstream-consumer evidence. Baseline `bin/test-fast`
  passed before the change on 2026-06-20. Focused `bash -n bin/common.sh`,
  focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_router_cli_consumer_smoke_exercises_raw_json_mcp_surface`,
  focused `git diff --check`, focused
  ...
  consumer-package smokes, the router-hosted MCP live public, pub/sub-only,
  authenticated, bearer, and JSON-response examples, the installed router CLI
  consumer smoke with explicit `jsonResponse.active.pubsub` summary evidence,
  full router tests, zero-copy router tests, and the Chrome/Dart2Wasm browser
  WebSocket smoke. Commit `bef2f08`
  (`test: expose json response mcp pubsub evidence`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27864737580` and GitHub `add-router` CI `27864733814` passed with
  `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry Run or
  WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest GitHub
  `master` Dart Package Publish Dry Run `27853666798`, GitHub `add-router`
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `bef2f08`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  browser WebSocket smoke. Commit `744aa9c`
  (`test: prove json response mcp auth lifecycle`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27863209435` and GitHub `add-router` CI `27863208948` passed with
  `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry Run or
  WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest GitHub
  `master` Dart Package Publish Dry Run `27853666798`, GitHub `add-router`
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `744aa9c`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  `lastEventId` after `deleteSession()`, aligning those flows with the
  existing token-only JSON-response and protected token-only cleanup checks.
  The emitted `routerCliConsumerSummary` now includes
  `streamableSessionDelete: true` for `public`, `secure`,
  `jsonResponse.active`, `jsonResponse.tokenOnly`, and `tokenOnly`, and the
  human-readable smoke summary names session delete coverage for the same
  route families. `tool/test_mcp_consumer_package_boundary.py` guards the new
  summary fragments, cleanup assertions, and summary text.
...
- 2026-06-20: Hardened the Cargo-sensitive bootstrap and verification paths
  ...
  coverage, and router native/auth/session tests. Full local `bin/verify`
  passed on 2026-06-20, including formatting, Rust/FFI tests, Python/tool
  tests, MCP package tests, generated consumer-package smokes, the
  router-hosted MCP live public, pub/sub-only, authenticated, bearer, and
  ...
  smoke. Commit `9d76819`
  (`ci: retry transient cargo bootstrap failures`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27860081238` and GitHub `add-router` CI `27860081246` passed with
  `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry Run or
  WAMP Profile Benchmarks were required because no publish-sensitive or
  benchmark-sensitive package inputs changed since `7202eaf`; the latest GitHub
  `master` Dart Package Publish Dry Run `27853666798`, GitHub `add-router`
  ...
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `9d76819`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  `notifyWampEventDirect` and `notifyWampEvent` on existing token-only
  subscriptions, and polls for matching events on token-only JSON-response
  direct/Streamable routes and protected token-only direct/Streamable routes.
  The emitted `routerCliConsumerSummary` now includes
  `pubsubNotifications: true` for both `jsonResponse.tokenOnly` and
  `tokenOnly`, separate from existing tool/RPC-triggered `pubsub` evidence.
  The human-readable smoke summary now names notification pub/sub coverage for
  both token-only route families, and
  `tool/test_mcp_consumer_package_boundary.py` guards the helper name, direct
  and Streamable notification helpers, JSON summary fragments, and summary
  text. Baseline `bin/test-fast` passed before the change on 2026-06-20.
...
- 2026-06-20: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  Dart helper `_expectConfiguredWampRegistrationMeta`, proves the
  route-configured protected procedure over token-only JSON-response
  direct/Streamable routes and protected token-only direct/Streamable routes,
  and emits `configuredRegistrationMeta: true` in `routerCliConsumerSummary`
  alongside registration/session/subscription/configured subscription metadata.
  The human-readable smoke summary now names configured registration evidence
  for both token-only route families, and
  `tool/test_mcp_consumer_package_boundary.py` guards the helper name, JSON
  summary fragments, and summary text. Baseline `bin/test-fast` passed before
  the change on 2026-06-20. Focused `bash -n bin/common.sh`, focused
  ...
  package tests, generated consumer-package smokes, the router-hosted MCP live
  public, pub/sub-only, authenticated, bearer, and JSON-response examples, the
  installed router CLI consumer smoke with explicit configured registration
  metadata summary fields, full router tests, zero-copy router tests, and the
  Chrome/Dart2Wasm browser WebSocket smoke. Hosted evidence for this installed
  router CLI configured registration summary checkpoint is clean. Commit
  `a565d46` (`test: expose configured registration in cli smoke summary`) was
  pushed to GitLab `origin`, GitHub `add-router`, and GitHub `master`. GitHub
  `master` CI `27855854493` and GitHub `add-router` CI `27855853922` passed
  with `Fast Checks` and `Full Verify` clean. No new Dart Package Publish Dry
  Run or WAMP Profile Benchmarks were required because this
  smoke-harness/state update did not change publish-sensitive or
  benchmark-sensitive inputs; the latest GitHub `master` Dart Package Publish
  Dry Run `27853666798`, GitHub `add-router` Dart Package Publish Dry Run
  `27853666197`, GitHub `master` WAMP Profile Benchmarks `27853666815`, and
  GitHub `add-router` WAMP Profile Benchmarks `27853666244` remain clean and
  relevant at `7202eaf`. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence at `a565d46`, and latest relevant Dart package dry-run evidence
  clean.
- 2026-06-20: The public router-hosted MCP client example now proves
  route-configured procedure registration metadata over direct JSON and
  Streamable HTTP before any live WAMP callee exists. The router-hosted server
  example declares `example.task.configured.lookup` as a route-configured
  procedure while keeping the live tool call on `example.task.lookup`; the
  client example exercises `wamp.registration.lookup`, `match`, `list`, `get`,
  `list_callees`, and `count_callees` through public direct JSON and Streamable
  helpers, asserts zero callees, and emits `configuredRegistrationMetadata` in
  dry-run/live summaries. `bin/common.sh` requires this evidence for public,
  authenticated, bearer, and JSON-response router-hosted client smokes, and
  `tool/test_mcp_consumer_package_boundary.py` guards the public example,
  server example, dry-run, and live-smoke wiring. Focused
  ...
  focused `bash -n bin/common.sh`, focused `git diff --check`, focused
  `python3 tool/check_public_artifact_references.py`, focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_public_router_hosted_client_example_uses_public_io_entrypoint tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_public_router_hosted_server_example_publishes_task_lookup_events tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_fast_smoke_runs_public_router_hosted_client_example_dry_run tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_fast_smoke_runs_public_router_hosted_client_example_live`,
  focused
  `bash -lc 'source bin/common.sh; run_public_router_hosted_mcp_client_dry_run_smoke'`,
  focused
  ...
  2026-06-20, including formatting, Rust/FFI tests, Python/tool tests, MCP
  package tests, generated consumer-package smokes, the router-hosted MCP live
  public, pub/sub-only, authenticated, bearer, and JSON-response examples with
  configured registration metadata assertions, the installed router CLI
  ...
  public-example configured registration metadata checkpoint is clean. Commit
  `7202eaf` (`test: prove configured mcp registration meta in public example`)
  was pushed to GitLab `origin`, GitHub `add-router`, and GitHub `master`.
  GitHub `master` CI `27853666797` and GitHub `add-router` CI `27853666194`
  passed with `Fast Checks` and `Full Verify` clean. GitHub `master` Dart
  Package Publish Dry Run `27853666798` and GitHub `add-router` Dart Package
  Publish Dry Run `27853666197` passed for the same commit. GitHub `master`
  WAMP Profile Benchmarks `27853666815` and GitHub `add-router` WAMP Profile
  Benchmarks `27853666244` passed for the same commit. The strict
  deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-20 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence, latest GitHub `master` WAMP benchmark evidence, and latest GitHub
  `master` Dart package dry-run evidence clean.
- 2026-06-19: Router-hosted MCP now exposes route-configured topics through
  standard WAMP subscription metadata even when no live WAMP subscriber session
  exists. `router_mcp.dart` synthesizes visible subscription entries for
  authorized route-configured topics, assigns stable synthetic ids from a
  deterministic high WAMP id range, and keeps live exact subscriptions on their
  real router ids. The router native integration smoke now proves
  `wamp.subscription.lookup`, `match`, `list`, `get`, `list_subscribers`, and
  `count_subscribers` for a configured read-only topic with no live subscriber,
  including zero visible subscribers and no synthetic-id collision with a later
  live subscription. `bin/common.sh` adds a configured-only protected topic to
  the installed router CLI consumer smoke and makes the generated Dart consumer
  assert configured subscription metadata on token-only JSON-response
  direct/Streamable and protected token-only direct/Streamable routes.
  `tool/test_mcp_consumer_package_boundary.py` guards the
  `configuredSubscriptionMeta` summary fragments and human-readable evidence
  text. Focused `bash -n bin/common.sh`, focused `git diff --check`, focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_router_cli_consumer_smoke_exercises_raw_json_mcp_surface`,
  focused
  ...
  including the configured subscription metadata assertions in the installed
...
- 2026-06-19: The public router-hosted MCP client example now proves
  ...
  `bash -lc 'source bin/common.sh; ensure_native_lib_env; run_public_router_hosted_mcp_client_live_smoke'`,
  post-change `bin/test-fast`, and full local `bin/verify` passed on
  2026-06-19. Commit `e52d75b`
  (`test: prove configured mcp subscription meta in public example`) was pushed
  to GitLab `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master`
  CI `27850764875` and GitHub `add-router` CI `27850763859` passed with
  `Fast Checks` and `Full Verify` clean. GitHub `master` Dart Package Publish
  Dry Run `27850764868` and GitHub `add-router` Dart Package Publish Dry Run
  `27850763870` passed for the same commit. The latest WAMP Profile Benchmarks
  remain GitHub `master` `27846722972` and GitHub `add-router` `27846718267` at
  `1c8ce29`; this public-example smoke checkpoint did not trigger
  benchmark-sensitive workflows. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-19 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence, and latest GitHub `master` Dart package dry-run evidence clean.
- 2026-06-19: Router-hosted MCP now exposes route-configured procedures through
  ...
  full router tests, zero-copy router tests, and the Chrome/Dart2Wasm browser
  WebSocket smoke. Commit `f0e60c0`
  (`fix: expose configured mcp registration meta`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27842851544` and GitHub `add-router` CI `27842846559` passed with
  `Fast Checks` and `Full Verify` clean. GitHub `master` Dart Package Publish
  Dry Run `27842851545`, GitHub `add-router` Dart Package Publish Dry Run
  `27842846587`, GitHub `master` WAMP Profile Benchmarks `27842851539`, and
  GitHub `add-router` WAMP Profile Benchmarks `27842846578` passed for the
  same commit. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-19 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
...
- 2026-06-19: Tightened the installed router CLI Dart MCP consumer summary so
  ...
  session/subscription metadata, pub/sub, and batch evidence in the
  `routerCliConsumerSummary` object. The summary now distinguishes active
  JSON-response auth isolation from token-only JSON-response resource/prompt,
  WAMP metadata, pub/sub, and batch access, and it marks token-only direct
  JSON/Streamable WAMP session and subscription metadata explicitly instead of
  relying on the human-readable completion line.
  ...
  consumer-facing evidence. Baseline `bin/test-fast` passed before the change
  on 2026-06-19. Focused `bash -n bin/common.sh`, focused
  `python3 -m unittest tool.test_mcp_consumer_package_boundary.McpConsumerPackageBoundaryTest.test_router_cli_consumer_smoke_exercises_raw_json_mcp_surface`,
  and focused
  `bash -lc 'source bin/common.sh; ensure_native_lib_env; run_router_cli_consumer_package_smoke'`
  passed on 2026-06-19. Post-change `bin/test-fast` passed on 2026-06-19,
  including the richer installed router CLI consumer summary assertion. Full
  local `bin/verify` passed on 2026-06-19, including formatting, Rust/FFI
  tests, Python/tool tests, MCP package tests, generated consumer-package
  smokes, the router-hosted MCP live public, pub/sub-only, authenticated,
...
- 2026-06-19: Tightened the installed router CLI Dart MCP consumer smoke so
  ...
  tests, and the Chrome/Dart2Wasm browser WebSocket smoke. Commit `c6b2cf5`
  ...
  clean. No new Dart Package Publish Dry Run was triggered for `c6b2cf5`
  because this smoke harness and boundary-test change did not touch
  publish-sensitive inputs; the latest GitHub `master` Dart Package Publish Dry
  Run `27824768142` and GitHub `add-router` Dart Package Publish Dry Run
  `27824767971` remain clean and relevant. No new WAMP Profile Benchmarks,
  Native Artifacts, or Router Image run was required because this MCP smoke
  change did not touch benchmark-sensitive, native artifact, image, or workflow
  inputs. The strict deployment-chain audit
...
- 2026-06-19: Added an opt-in public router-hosted MCP ticket-auth lifecycle
  ...
  grant, refreshes it, proves the refreshed grant works for direct JSON `ping`
  without Streamable state, proves it can initialize and notify an initialized
  Streamable session, deletes that session, revokes the refreshed access token,
  verifies direct JSON rejects the revoked token with HTTP 401 without creating
  session state, revokes the refreshed refresh token, and verifies refresh
  rejects the revoked token with HTTP 401. Dry-run output reports
  `authLifecycleSmoke: true` without printing ticket or token material.
  `bin/common.sh` covers the new parser guard, dry-run summary field, and live
  authenticated router-hosted MCP and JSON-response examples.
  `tool/test_mcp_consumer_package_boundary.py` guards the public option, helper
  calls, exception types, poll ids, output fields, and smoke invocations.
  Baseline `bin/test-fast` passed before the change on 2026-06-19. Focused
  `dart format packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `python3 tool/test_mcp_consumer_package_boundary.py`, focused
  `python3 tool/check_public_artifact_references.py`, focused
...
- 2026-06-19: Tightened the public router-hosted MCP examples so
  ...
  the installed router CLI consumer smoke, full router tests, zero-copy router
  tests, and the Chrome/Dart2Wasm browser WebSocket smoke. Commit `05c8abe`
  ...
  Benchmarks `27818081306` also passed, although no benchmark-sensitive input
  was required for this MCP change. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --run-limit 6 --require-clean-latest-ci --show-dart-package-publish-dry-run --require-clean-dart-package-publish-dry-run --strict`
  exited successfully on 2026-06-19 with branch protection, workflow
  visibility, router image package visibility, latest GitHub `master` CI
  evidence, and latest GitHub `master` Dart package dry-run evidence clean. No
  new Native Artifacts or Router Image run was required because no native
  artifact, image, or workflow inputs changed.
...
- 2026-06-19: Tightened the public router-hosted MCP client example so raw
  ...
  `packages/connectanum_mcp/example/router_hosted_client.dart` now uses
  `postDirect` inside `_runDirectJsonExample` for `resources/list`,
  `resources/templates/list`, `resources/read`, `prompts/list`, and
  `prompts/get`, validates the configured resource URI and prompt name in the
  raw catalogs, and emits `directResourceMethodResources`,
  `directResourceMethodTemplates`, `directResourceMethodContent`,
  `directPromptMethodCatalog`, and `directPromptMethod`. The Streamable
  session path mirrors the same standard methods with `post` and emits
  `resourceMethods` and `promptMethods`. The new `_responseResult` helper
  rejects missing responses, mismatched ids, JSON-RPC errors, and non-object
  results before the example can report success.
  `tool/test_mcp_consumer_package_boundary.py` guards the new raw method ids,
  labels, helper, and output fields so future edits cannot silently remove
  single-request direct JSON or Streamable resource/prompt coverage from the
  public example. Baseline `bin/test-fast` passed before the change on
  2026-06-19. Focused
  `dart format packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `python3 tool/test_mcp_consumer_package_boundary.py`, focused
  ...
  single direct JSON and Streamable resource/prompt method checks, direct JSON
  and Streamable pub/sub notification side-effect checks, raw direct JSON
  pub/sub method publish checks, raw Streamable pub/sub method publish checks,
  raw Streamable tool/API method checks, raw direct tool method checks, raw
  direct WAMP method metadata checks, direct JSON lifecycle, session
...
- 2026-06-19: Tightened the public router-hosted MCP client example so
  ...
  now calls `notifyWampEventDirect` and
  `notifyConnectanumMethodDirect('connectanum.pubsub.publish')` inside
  `_runDirectPubSubExample`, then polls the existing direct subscription with
  `direct-pubsub-notification-poll` and
  `direct-pubsub-method-notification-poll` for distinct notification events.
  `_runStreamableSessionExample` mirrors the same coverage with
  `notifyWampEvent`, `notifyConnectanumMethod`, and corresponding Streamable
  poll ids. The public JSON output now includes `notificationEvents` and
  `methodNotificationEvents` for both flows.
  `tool/test_mcp_consumer_package_boundary.py` guards the new notification
  poll ids, helper calls, labels, and output fields so future edits cannot
  silently remove notification-style pub/sub side-effect coverage from the
  public example. Baseline `bin/test-fast` passed before the change on
  2026-06-19. Focused
  `dart format packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  ...
  generated consumer-package smokes, the router-hosted MCP live public,
  pub/sub-only, authenticated, bearer, and JSON-response examples with direct
  JSON and Streamable pub/sub notification side-effect checks, raw direct JSON
  pub/sub method publish checks, raw Streamable pub/sub method publish checks,
  raw Streamable tool/API method checks, raw direct tool method checks, raw
  direct WAMP method metadata checks, direct JSON lifecycle, session
  initialization, session deletion, pub/sub metadata, standard MCP catalog, and
  WAMP catalog self-checks, the installed router CLI consumer smoke, full
  router tests, zero-copy router tests, and Chrome/Dart2Wasm browser WebSocket
  smoke. Commit `939e1ef` (`test: assert mcp pubsub notifications`) was pushed
  to GitLab `origin`, GitHub `add-router`, and GitHub `master`. GitHub
  `master` CI `27803209989` and GitHub `add-router` CI `27803210005` passed
  with `Fast Checks` and `Full Verify` clean. GitHub `master` Dart Package
  Publish Dry Run `27803209985` and GitHub `add-router` Dart Package Publish
  Dry Run `27803210002` also passed. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-19 with branch protection, workflow visibility,
  router image package visibility, latest GitHub `master` CI evidence, and
  latest GitHub `master` Dart package dry-run evidence clean. No new Native
  ...
  no native artifact, image, workflow, or benchmark-sensitive inputs changed.
- 2026-06-19: Tightened the public router-hosted MCP client example so raw
  dotted direct JSON method calls also prove pub/sub publish readiness, not only
  typed direct pub/sub helpers and the stateful Streamable method path.
  ...
  `_runDirectPubSubExample` after the typed direct publish and poll, validates
  the raw method publication returns the configured topic, acknowledgement, and
  publication id, then polls the same direct subscription for a distinct
  method-published event. The public JSON output now includes
  `methodPublication` and `methodEvents`.
  `tool/test_mcp_consumer_package_boundary.py` guards the new raw direct JSON
  pub/sub method id, poll id, and labels so future edits cannot silently remove
  lifecycle-free direct method-style pub/sub coverage from the public example.
  Baseline `bin/test-fast` passed before the change on 2026-06-19. Focused
  `dart format packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `python3 tool/test_mcp_consumer_package_boundary.py`, focused
  ...
  generated consumer-package smokes, the router-hosted MCP live public,
  pub/sub-only, authenticated, bearer, and JSON-response examples with raw
  direct JSON pub/sub method publish checks, raw Streamable pub/sub method
  publish checks, raw Streamable tool/API method checks, raw direct tool method
  checks, raw direct WAMP method metadata checks, direct JSON lifecycle,
  ...
  smoke, full router tests, zero-copy router tests, and Chrome/Dart2Wasm
  browser WebSocket smoke. Commit `a176d89`
  (`test: assert direct mcp pubsub method`) was pushed to GitLab `origin`,
  GitHub `add-router`, and GitHub `master`. GitHub `master` CI `27800452634`
  and GitHub `add-router` CI `27800448873` passed with `Fast Checks` and
  `Full Verify` clean. GitHub `master` Dart Package Publish Dry Run
  `27800452588` and GitHub `add-router` Dart Package Publish Dry Run
...
- 2026-06-19: Tightened the public router-hosted MCP client example so raw
  ...
  hosted checkpoint is now `147d286`
  (`test: assert streamable mcp pubsub method`). Commit `147d286` was pushed to
  GitLab `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master`
  CI `27798203809` and GitHub `add-router` CI `27798199559` passed with
  `Fast Checks` and `Full Verify` clean. GitHub `master` Dart Package Publish
  Dry Run `27798203813` and GitHub `add-router` Dart Package Publish Dry Run
  `27798199556` also passed. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-19 with branch protection, workflow visibility,
  router image package visibility, latest GitHub `master` CI evidence, and
  latest GitHub `master` Dart package dry-run evidence clean. No new Native
...
- 2026-06-19: Tightened the public router-hosted MCP client example so raw
  ...
  calls `callConnectanumMethod` inside `_runStreamableSessionExample` for
  `connectanum.tools.list`, `connectanum.tool.call`, `connectanum.api.list`,
  and `connectanum.api.describe` after `initialize` and
  `notifications/initialized`. The public example validates the raw Streamable
  method tool catalog contains the configured `--tool`, asserts typed and raw
  Streamable tool calls do not return `isError`, validates raw Streamable WAMP
  procedure/topic catalogs and descriptions for the configured
  procedure/topic, emits `toolMethodCatalog`, `toolMethodResult`,
  `methodCatalog`, and `methodDescription`, and keeps the Streamable
  session-id invariant around the batched calls.
  `tool/test_mcp_consumer_package_boundary.py` guards the new Streamable raw
  method ids, labels, and output fields so future edits cannot silently remove
  stateful direct method-style tool/API coverage from the public example.
  Baseline `bin/test-fast` passed before the change on 2026-06-19. Focused
  `dart format packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `python3 tool/test_mcp_consumer_package_boundary.py`, focused
...
- 2026-06-18: Tightened the public router-hosted MCP client example so raw
  ...
  `packages/connectanum_mcp/example/router_hosted_client.dart` now calls
  `callConnectanumMethodDirect` for `connectanum.tools.list` and
  `connectanum.tool.call` inside `_runDirectJsonExample`, validates the raw
  method catalog contains the configured `--tool`, asserts both typed and raw
  method tool calls do not return `isError`, emits `directToolMethodCatalog`
  and `directToolMethodResult`, and keeps the direct JSON Streamable state
  guard around typed helper and raw method access.
  `tool/test_mcp_consumer_package_boundary.py` guards the new raw tool method
  ids, labels, helper, and output fields so future edits cannot silently remove
  direct method-style tool API coverage from the public example. Baseline
  ...
  generated consumer-package smokes, the router-hosted MCP live public,
  pub/sub-only, authenticated, bearer, and JSON-response examples with raw
  direct tool method checks, raw direct WAMP method metadata checks, direct
  JSON lifecycle, session initialization, session deletion, pub/sub metadata,
  standard MCP catalog, and WAMP catalog self-checks, the installed router CLI
  consumer smoke, full router tests, zero-copy router tests, and
  Chrome/Dart2Wasm browser WebSocket smoke. Commit `92e86bf`
  (`test: assert router mcp raw tool methods`) was pushed to GitLab `origin`,
  GitHub `add-router`, and GitHub `master`. GitHub `master` CI `27792708835`
  and GitHub `add-router` CI `27792704354` passed with `Fast Checks` and
  `Full Verify` clean. GitHub `master` Dart Package Publish Dry Run
  `27792708806` and GitHub `add-router` Dart Package Publish Dry Run
  `27792704358` also passed. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  ...
  no native artifact, image, workflow, or benchmark-sensitive inputs changed.
- 2026-06-18: Tightened the public router-hosted MCP client example so raw
  dotted direct JSON-RPC method calls prove WAMP metadata access, not only
  typed direct helpers and `connectanum.tool.call` wrappers.
  `packages/connectanum_mcp/example/router_hosted_client.dart` now calls
  `callConnectanumMethodDirect` for `connectanum.api.list` and
  `connectanum.api.describe` inside `_runDirectWampMetadataExample`, validates
  the raw method `structuredContent` for the configured WAMP procedure/topic
  catalogs and descriptions, emits `methodCatalog` and `methodDescription`,
  and keeps the direct WAMP metadata Streamable state guard around typed helper
  and raw method access. `tool/test_mcp_consumer_package_boundary.py` guards
  the new raw method ids, labels, helper, and structured-content error text so
  future edits cannot silently remove direct method-style WAMP metadata
  coverage from the public example. Baseline `bin/test-fast` passed before the
  ...
  bearer, and JSON-response examples with raw direct WAMP method metadata
  checks, direct JSON lifecycle, session initialization, session deletion,
  pub/sub metadata, standard MCP catalog, and WAMP catalog self-checks,
  generated consumer-package smokes, the installed router CLI consumer smoke,
  full router tests, zero-copy router tests, and Chrome/Dart2Wasm browser
  WebSocket smoke. Commit `48d44b8`
  (`test: assert router mcp raw metadata methods`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27789200414` and GitHub `add-router` CI `27789195095` passed with
  `Fast Checks` and `Full Verify` clean. GitHub `master` Dart Package Publish
  Dry Run `27789200393` and GitHub `add-router` Dart Package Publish Dry Run
  `27789195118` also passed. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-18 with branch protection, workflow visibility,
  router image package visibility, latest GitHub `master` CI evidence, and
  latest GitHub `master` Dart package dry-run evidence clean. No new Native
...
- 2026-06-18: Tightened the public router-hosted MCP client example so direct
  ...
  Chrome/Dart2Wasm browser WebSocket smoke. Commit `32750be`
  (`test: assert router mcp lifecycle state`) was pushed to GitLab `origin`,
  GitHub `add-router`, and GitHub `master`. GitHub `master` CI `27785333492`
  and GitHub `add-router` CI `27785333266` passed with `Fast Checks` and
  `Full Verify` clean. GitHub `master` Dart Package Publish Dry Run
  `27785333568` and GitHub `add-router` Dart Package Publish Dry Run
  `27785333268` also passed. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-18 with branch protection, workflow visibility,
  router image package visibility, latest GitHub `master` CI evidence, and
  latest GitHub `master` Dart package dry-run evidence clean. No new Native
...
- 2026-06-18: Tightened the public router-hosted MCP client example so
  ...
  now shares `_expectWampSubscription`, `_expectWampPublication`, and
  `_expectWampEventBatch` across direct JSON and Streamable paths. The public
  example asserts subscription topic, non-empty handle, requested queue limit,
  acknowledged publication topic/id, polled handle/topic, zero dropped and
  remaining events, and the expected event payload before reporting pub/sub
  output. Direct JSON pub/sub output now separates runtime subscription details
  from `subscriptionMetadata`, matching the Streamable output shape.
  `tool/test_mcp_consumer_package_boundary.py` guards the helper names, direct
  and Streamable pub/sub labels, and failure text so future edits cannot
  silently reduce the consumer-facing smoke back to payload-only coverage.
...
- 2026-06-18: Tightened the public router-hosted MCP client example so
  ...
  The latest fully clean hosted checkpoint is `ce2f8b0`: commit `ce2f8b0`
  (`test: assert router mcp batch catalogs`) was pushed to GitLab `origin`,
  GitHub `add-router`, and GitHub `master`. GitHub `master` CI `27777433586`
  and GitHub `add-router` CI `27777433893` passed with `Fast Checks` and
  `Full Verify` clean. GitHub `master` Dart Package Publish Dry Run
...
- 2026-06-18: Tightened the public router-hosted MCP client example so WAMP
  ...
  `packages/connectanum_mcp/example/router_hosted_client.dart` now verifies
  that direct JSON `listWampApiDirect` and Streamable `listWampApi` catalogs
  include the requested procedure/topic URI before continuing to describe,
  registration, subscription, and pub/sub helpers. This makes the public live
  example fail fast if public, protected, bearer-token, or protected
  JSON-response routes expose describe/pub-sub behavior but lose catalog
  discovery. `tool/test_mcp_consumer_package_boundary.py` guards the helper,
  error template, and direct/Streamable labels. Baseline `bin/test-fast` passed
  before the change on 2026-06-18. Focused
  `dart analyze packages/connectanum_mcp/example/router_hosted_client.dart`,
  focused `python3 -m unittest tool/test_mcp_consumer_package_boundary.py`,
  focused `git diff --check`, focused
...
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  zero-copy router tests, and Chrome/Dart2Wasm browser WebSocket smoke. The
  latest fully clean hosted checkpoint is `3ec6b3f`: commit `3ec6b3f`
  (`test: cover json response mcp procedure catalog`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27763851453` and GitHub `add-router` CI `27763850438` passed with
  `Fast Checks` and `Full Verify` clean. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-18 with branch protection, workflow visibility,
...
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  `describeWampApi` for `cli.smoke.secure.lookup` and
  `cli.smoke.secure.events`, asserts the `CLI Secure Lookup` and
  `CLI Secure Smoke Events` metadata, and asserts the active session id and
  resume cursor remain unchanged. The token-only JSON-response Streamable
  client now repeats the procedure describe path before its existing topic,
  resource, prompt, batch, and pub/sub checks. This covers consumer
  applications that use standard Streamable MCP tool calls against a
  JSON-response route instead of the default protected `/mcp/secure` route.
  `tool/test_mcp_consumer_package_boundary.py` guards the new ids, assertion
  text, and updated smoke success summary. Baseline `bin/test-fast` passed
...
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  is active, the generated consumer calls `describeWampApiDirect` for
  `cli.smoke.secure.lookup` and asserts the `CLI Secure Lookup` metadata
  without mutating the active session id or resume cursor. The token-only
  JSON-response direct client repeats the procedure describe path before its
  existing topic, resource, prompt, batch, and pub/sub checks. This covers
  consumer applications that use the protected JSON-response endpoint for
  lifecycle-free direct WAMP metadata access instead of the default protected
  `/mcp/secure` endpoint. `tool/test_mcp_consumer_package_boundary.py` guards
  the new ids, assertion text, and updated smoke success summary. Baseline
  `bin/test-fast` passed before the change on 2026-06-18. Focused
  `bash -n bin/common.sh`, focused
  `python3 -m unittest tool/test_mcp_consumer_package_boundary.py`, focused
  `git diff --check`, and focused
  ...
  generated consumer-package smokes, the router-hosted MCP live public,
  pub/sub-only, authenticated, bearer, and JSON-response examples, the
  installed router CLI consumer smoke with protected JSON-response direct WAMP
  procedure metadata parity, full router tests, zero-copy router tests, and
  Chrome/Dart2Wasm browser WebSocket smoke. The latest fully clean hosted
  checkpoint is `1f41d5f`: commit `1f41d5f` (`test: cover json response mcp
  procedure metadata`) was pushed to GitLab `origin`, GitHub `add-router`, and
  GitHub `master`. GitHub `master` CI `27756308128` and GitHub `add-router` CI
  `27756300607` passed with `Fast Checks` and `Full Verify` clean. The strict
...
- 2026-06-18: Extended the installed router CLI consumer smoke with declared
  ...
  router tests, zero-copy router tests, and Chrome/Dart2Wasm browser WebSocket
  smoke. The latest fully clean hosted checkpoint is `1153683`: commit
  `1153683` (`test: cover mcp procedure metadata smoke`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. GitHub `master` CI
  `27752782568` and GitHub `add-router` CI `27752776438` passed with `Fast
...
- 2026-06-18: Extended the installed router CLI consumer smoke with
  ...
  Streamable session, asserts `tools/list` exposes `connectanum.api.describe`,
  calls it through `tools/call` for `cli.smoke.events`, and asserts the `CLI
  Smoke Events` metadata before Streamable pub/sub delivery. The
  bearer-protected `/mcp/secure` smoke repeats the same standard Streamable
  tool-call path for `cli.smoke.secure.events` and asserts `CLI Secure Smoke
  Events` metadata before protected Streamable pub/sub delivery. This covers
  agents and consumer applications that use standard Streamable HTTP MCP tool
  calls for WAMP metadata without relying on lifecycle-free direct JSON calls or
  generated Dart helper assumptions. `tool/test_mcp_consumer_package_boundary.py`
  guards the new Streamable topic describe ids, assertion text, and updated
  smoke success summary. Baseline `bin/test-fast` passed before the change on
  2026-06-18. Focused `bash -n bin/common.sh`, focused
  `python3 -m unittest tool/test_mcp_consumer_package_boundary.py`, focused
  `git diff --check`, focused `python3 tool/check_public_artifact_references.py`,
  and focused
  `bash -lc 'source bin/common.sh; run_router_cli_consumer_package_smoke'`
...
- 2026-06-18: Extended the installed router CLI consumer smoke with direct raw
  ...
  posts `connectanum.api.describe` and asserts the `CLI Smoke Events` metadata
  is returned before direct pub/sub plus Streamable pub/sub delivery. The
  bearer-protected `/mcp/secure` smoke repeats the direct raw JSON describe
  check for `cli.smoke.secure.events` and asserts the `CLI Secure Smoke Events`
  metadata before protected direct pub/sub and Streamable pub/sub delivery.
  This covers agents and consumer applications that use the router-provided
  direct JSON tool/meta API surface without generated Dart helper assumptions.
  `tool/test_mcp_consumer_package_boundary.py` guards the raw JSON describe ids,
  assertion text, and updated smoke success summary. Baseline `bin/test-fast`
  passed before the change on 2026-06-18. Focused `bash -n bin/common.sh`,
  focused `python3 -m unittest tool/test_mcp_consumer_package_boundary.py`,
  focused `git diff --check`, focused
  `python3 tool/check_public_artifact_references.py`, and focused
  `bash -lc 'source bin/common.sh; run_router_cli_consumer_package_smoke'`
  ...
  router tests, zero-copy router tests, and Chrome/Dart2Wasm browser WebSocket
  smoke. Commit `0b233dd` (`test: cover raw json mcp topic describe`) was
  pushed to GitLab `origin`, GitHub `add-router`, and GitHub `master`. Hosted
  evidence is clean at `0b233dd`: GitHub `master` CI `27744471480` and GitHub
  `add-router` CI `27744471455` passed with `Fast Checks` and `Full Verify`
  clean. The strict deployment-chain audit
  `bin/audit-github-deployment-chain --branch master --strict` exited
  successfully on 2026-06-18 with branch protection, workflow visibility,
  router image package visibility, and latest `master` CI evidence clean. No
  new Dart Package Publish Dry Run, Native Artifacts, Router Image, or WAMP
  Profile Benchmarks run was required because no package, workflow, native
  artifact, image, or benchmark inputs changed.
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  browser WebSocket smoke. Commit `c3190c8`
  (`test: cover active json-response mcp isolation`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. Hosted evidence is clean
  at `c3190c8`: GitHub `master` CI `27741271360` and GitHub `add-router` CI
  `27741267375` passed with `Fast Checks` and `Full Verify` clean. The strict
  deployment-chain audit `bin/audit-github-deployment-chain --branch master
  --strict` exited successfully on 2026-06-18 with branch protection, workflow
  visibility, router image package visibility, and latest `master` CI evidence
  clean. No new Dart Package Publish Dry Run, Native Artifacts, Router Image, or
  WAMP Profile Benchmarks run was required because no package, workflow, native
  artifact, image, or benchmark inputs changed. RC readiness remains gated on
  release policy: no numeric RC tag points at `c3190c8`, the existing
  `v0.1.0-rc.1` tag still points at stale commit `47bbf9c`, no GitHub
  prerelease or router image RC tag is selected for `c3190c8`, and pub.dev
...
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  `listResourceTemplatesDirect`, `listPromptsDirect`, and `getPromptDirect`
  against the protected route, asserts the secure task template and prompt
  substitution are visible, and asserts the direct JSON helpers do not mutate
  the valid client's session id or resume cursor. This covers consumer
  applications that mix authenticated Streamable sessions with lifecycle-free
  direct JSON resource and prompt helper calls through public
  `connectanum_mcp_io` APIs. The installed CLI smoke summary now reports
  `active protected direct JSON WAMP meta and resource/prompt isolation`, and
...
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
  ...
  Chrome/Dart2Wasm browser WebSocket smoke. Commit `96a1ba4`
  (`test: cover active protected mcp meta isolation`) was pushed to GitLab
  `origin`, GitHub `add-router`, and GitHub `master`. Hosted evidence is clean
  at `96a1ba4`: GitHub `master` CI `27736430852` and GitHub `add-router` CI
  `27736427769` passed with `Fast Checks` and `Full Verify` clean. No new Dart
  Package Publish Dry Run, Native Artifacts, Router Image, or WAMP Profile
  Benchmarks run was required because no inputs sensitive to those workflows
  changed; the strict audit accepted the latest relevant successful runs. The
  strict deployment-chain audit exited successfully on 2026-06-18 with clean
  latest CI logs at `96a1ba4`, relevant Dart Package Publish Dry Run
  `27724124864` at `52dba41`, relevant Native Artifacts dry-run `26396437881`
  at `debd545`, relevant Router Image dry-run `27550613315` at `77f22c0`,
  relevant WAMP Profile Benchmarks `27549457823` at `77f22c0`, branch
  protection, workflow visibility, and router image package visibility gates
  ready. RC readiness remains gated on release policy: no numeric RC tag points
  at `96a1ba4`, the existing `v0.1.0-rc.1` tag still points at stale commit
  `47bbf9c`, the audit suggests `v0.1.0-rc.2` as the next numeric tag if
  release policy approves it, no GitHub prerelease or router image RC tag is
  selected for `96a1ba4`, and pub.dev package ownership/version/release-order
  decisions remain deferred.
- 2026-06-18: Extended the installed router CLI generated Dart consumer smoke
...
malformed `MCP-Session-Id` rejection while a Streamable HTTP session is open.
The active direct path reruns the raw malformed-session probe after
lifecycle-free direct JSON tool/resource/prompt/WAMP/batch operations, records
...
client dry-run/live smoke; and `git diff --check` passed on 2026-07-09. Full
local `bin/verify` passed after the active direct malformed-session change on
2026-07-09. After commit `784bbfa`, the first hosted GitHub CI run
...
complete for commit `6cdd0a7`: GitHub CI `29004177744` and Dart Package
...
JSON metadata batches, and now also calls `connectanum.api.list` and
`connectanum.api.describe` through lifecycle-free direct JSON method helpers.
The example validates the returned procedure/topic catalogs and descriptions,
surfaces `methodCatalog` and `methodDescription` under
`activeDirectJson.wampMetadata.procedure` and `.topic`, and keeps asserting the
...
`bin/verify` passed on 2026-07-09. Hosted evidence for this checkpoint: after
commit `c792304`, hosted GitHub CI `28997186100` and Dart Package Publish Dry
Run `28997186077` passed on 2026-07-09. The deployment-chain audit with
required clean latest CI, clean CI logs, Dart package publish dry-run, and WAMP
profile benchmark evidence also passed at `c792304`; WAMP Profile Benchmarks
`28964007707` remain clean and relevant from `b81302a` because no WAMP
benchmark-sensitive paths changed since then. The audit noted GitHub Actions
degraded performance from delayed starts, but still reports only expected
operator-owned gaps: `add-router` is unprotected, and the checked-in pub.dev
OIDC workflows are not Actions discoverable until promoted through `master`.
...
notification-only pub/sub variants, and now also sends standard
`notifyToolDirect`, Connectanum `notifyConnectanumToolDirect`, and raw
`connectanum.tool.call` notification variants against the example task lookup
tool. The smoke polls the resulting WAMP events, validates the emitted task
lookup payloads, surfaces `toolNotificationEvents`,
`connectanumToolNotificationEvents`, and `toolMethodNotificationEvents` in the
active direct JSON pub/sub summary, and keeps asserting the Streamable session
...
After commit `fb4cb6f`, hosted GitHub CI `28992372472` and Dart Package
Publish Dry Run `28992372478` passed on 2026-07-09. The deployment-chain audit
with required clean latest CI, clean CI logs, Dart package publish dry-run, and
WAMP profile benchmark evidence also passed at `fb4cb6f`; WAMP Profile
Benchmarks `28964007707` remain clean and relevant from `b81302a` because no
WAMP benchmark-sensitive paths changed since then. Strict audit still reports
only expected operator-owned gaps: `add-router` is unprotected, and the
...
GitHub prerelease/router-image tag selection, and deferred pub.dev
release-order decisions. No RC tag, GitHub Release, router image, or pub.dev
publication was created or moved by this checkpoint.
