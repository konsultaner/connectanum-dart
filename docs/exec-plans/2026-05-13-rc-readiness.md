...
- 2026-06-13: Added live fast-smoke coverage for the checked-in public
  router-hosted MCP client example. `run_router_hosted_mcp_example_smoke` still
  ...
  long-lived server, waits for the public endpoint line, invokes the public
  client live against that endpoint with direct JSON tool/resource/prompt and
  pub/sub options, and shuts the server down through bounded cleanup with a
  SIGKILL fallback plus server-log output on startup failure or timeout. This
  proves the consumer-facing example can use router-hosted MCP through
  `package:connectanum_mcp/connectanum_mcp_io.dart` without private project
  imports or a hand-maintained endpoint. `tool/test_mcp_consumer_package_boundary.py`
  now guards both the dry-run and live-smoke wiring so the fast path keeps
  exercising the public package boundary. Baseline `bin/test-fast` passed
  before the change on 2026-06-13. Focused `bash -n bin/common.sh`, focused
  ...
  evidence remains clean at the previous pushed checkpoint `b9556f5` until this
  local checkpoint is pushed and hosted checks complete: GitHub `master` CI
  `27477985313` passed with `Fast Checks`
  and `Full Verify` clean, GitHub `add-router` CI `27477982334` also passed at
  `b9556f5`, GitHub `master` Dart Package Publish Dry Run `27477985337` passed
  at `b9556f5`, and GitHub `add-router` Dart Package Publish Dry Run
  `27477982328` also passed at `b9556f5`. The strict deployment-chain audit
  exited successfully on 2026-06-13 with clean latest CI logs, relevant Dart
  package publish dry-run `27477985337` at `b9556f5`, relevant Native
  ...
  `b9556f5`, the existing `v0.1.0-rc.1` tag still points at stale commit
  `47bbf9c`, no GitHub prerelease or router image RC tag is selected for
  `b9556f5`, the audit suggests `v0.1.0-rc.2` only after release approval, and
