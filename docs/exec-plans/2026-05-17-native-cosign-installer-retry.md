...
failures before signing native bundles.
...
- Private-name scan on the touched workflow/state/plan paths passed on
  2026-05-17.
- `bin/verify` passed after edits on 2026-05-17.
...
- Local verification is clean. Because this is workflow-sensitive, hosted
  evidence should be refreshed after the implementation commit is pushed.
