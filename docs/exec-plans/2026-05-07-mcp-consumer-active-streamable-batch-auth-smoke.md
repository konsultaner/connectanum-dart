# Exec Plan: MCP Consumer Active Streamable Batch Auth Smoke
...
Prove from the generated neutral consumer package that Streamable JSON-RPC batch
...
- In scope:
  ...
  - Exercise public `McpStreamableHttpClient.postBatch` on an already
    initialized secure Streamable client after its bearer token has been rotated
    or revoked.
  - Assert the active-session Streamable batch POST is rejected with HTTP 401.
  - Assert the public client clears stale Streamable session id and SSE cursor
    state after the rejected batch.
  - Keep the existing direct JSON batch, direct JSON single,
  ...
  - Bundle existing hosted-evidence docs updates from the previous MCP active
    notification auth smoke checkpoint.
- Out of scope:
...
- Existing docs-only hosted-evidence updates for the previous MCP active
  notification auth smoke plan.

## Preconditions

- Latest pushed implementation commit `1bcb6c9` has clean hosted CI evidence.
...
1. Add an active-session Streamable JSON-RPC batch auth rejection assertion to
...
2. Reuse the existing active Streamable rejected-bearer harness so direct JSON
   ...
   Streamable response POST, GET/SSE, and DELETE are covered together.
...
- Commit `b355f84` (`test: cover mcp active streamable batch auth`) was pushed
  to `origin/add-router` and `github/add-router` on 2026-05-07.
- Hosted GitHub `CI` run `25519775921` completed successfully on 2026-05-07
...
- 2026-05-07: Chose this slice because active-session Streamable batch POSTs
  are a distinct MCP request shape from direct JSON batches, single JSON-RPC
  requests, notification-only POSTs, response POSTs, GET/SSE polls, and DELETE
  requests.
