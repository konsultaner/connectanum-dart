...
class SocketTransport extends AbstractTransport {
  ...
  Future<void> _runPingInterval() async {
    if (_pingInterval != null) {
      ...
      var pingIntervalInMilliseconds = _pingInterval?.inMilliseconds as int;
      if (isReady) {
        unawaited(sendPing(
                timeout: Duration(
                    milliseconds:
                        (pingIntervalInMilliseconds * 2 / 3).floor()))
  ...
  Stream<AbstractMessage>? receive() {
    ...
    return _socket
        ?.where((List<int> message) {
          ...
          if (_negotiateProtocol(Uint8List.fromList(message)) || !_assertValidMessage(Uint8List.fromList(message))) {
            return false;
          }
          final finalMessageLength = message.length - headerLength;
          final payloadLength =
              SocketHelper.getPayloadLength(Uint8List.fromList(message), headerLength);
          if (finalMessageLength < payloadLength) {
            _inboundBuffer = Uint8List.fromList(message);
            return false;
          }
          if (_messageLength != null && finalMessageLength > _messageLength!) {
  ...
  Future<Uint8List?> sendPing({Duration? timeout}) async {
    if (_pingCompleter == null || (_pingCompleter?.isCompleted != null && _pingCompleter?.isCompleted == true)) {
  ...
  void send(AbstractMessage message) {
    ...
    if (!_handshakeCompleter.isCompleted) {
      if (_outboundBuffer != null && _outboundBuffer?.isEmpty != null && _outboundBuffer?.isEmpty == true) {
      ...
      if(_outboundBuffer != null) {
        ...
        if(serialalizedMessage != null) {
          var outboundBufferIntList = _outboundBuffer?.toList() as List<int>;
          outboundBufferIntList += SocketHelper.buildMessageHeader(
              SocketHelper.MESSAGE_WAMP,
              serialalizedMessage.length,
              isUpgradedProtocol);
          outboundBufferIntList += serialalizedMessage;
          _outboundBuffer = Uint8List.fromList(outboundBufferIntList);
        }
      }
    } else {
      var serialalizedMessage = _serializer.serialize(message);
