...
class Session {
  ...
  final StreamController _openSessionStreamController = StreamController.broadcast();
    ...
    session._transportStreamSubscription = transport!.receive()?.listen(
        (message) {
          if (message is Challenge) {
            var foundAuthMethod = authMethods != null ? authMethods
            ...
            if(session._transportStreamSubscription != null) {
    ...
    if (subscribed is Subscribed) {
      ...
      subscribed.eventStream =
        ...
        if (message is Unsubscribed &&
          ...
          subscribed.revoke(message.details != null ? message.details!.reason : 'reason unknown');
          return false;
        }
