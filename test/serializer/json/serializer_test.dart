...
void main() {
  ...
  group('unserialize', () {
    ...
    test('Event', () {
      var event = serializer.deserializeFromString('[36, 5512315355, 4429313566, {}]') as Event?;
      ...
      event = serializer.deserializeFromString('[36, 5512315355, 4429313566, {}, [30]]') as Event?;
      expect(event, isNotNull);
      expect(event!.id, equals(MessageTypes.CODE_EVENT));
      expect(event.subscriptionId, equals(5512315355));
      expect(event.publicationId, equals(4429313566));
      ...
      event = serializer.deserializeFromString('[36, 5512315355, 4429313566, {}, ["johnny"], {"userid": 123, "karma": 10}]') as Event?;
      expect(event, isNotNull);
      expect(event!.id, equals(MessageTypes.CODE_EVENT));
      expect(event.subscriptionId, equals(5512315355));
      expect(event.publicationId, equals(4429313566));
  ...
  group('string conversion', () {
    test('convert UTF-8', () {
      ...
      var serializedInvocation = serializer.deserialize(serializer.serialize(invocation)) as Invocation?;
    ...
    test('convert json binary string', () {
      ...
      expect(result.argumentsKeywords!['binary']['content'][0], isA<Uint8List>());
      expect(result.argumentsKeywords!['binary']['content'][0].length, equals(16));
      expect(result.argumentsKeywords!['binary']['content'][1], isA<String>());
      result = serializer.deserializeFromString('[50, 1, {}, "\\u0000EOP/kFMHXFJvX8BtT+N82w=="]') as Result?;
