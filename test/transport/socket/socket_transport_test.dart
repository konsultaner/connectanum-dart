...
void main() {
  group('Socket protocol negotiation', () {
    ...
    test('Opening with server error', () async {
      ...
      server.listen((socket) {
        socket.listen((message) {
          ...
          if (SocketHelper.getMaxMessageSizeExponent(message) == 10) {
            socket.add(SocketHelper.getError(SocketHelper.ERROR_USE_OF_RESERVED_BITS));
          }
          if (SocketHelper.getMaxMessageSizeExponent(message) == 11) {
            socket.add(SocketHelper.getError(SocketHelper.ERROR_MESSAGE_LENGTH_EXCEEDED));
          }
          if (SocketHelper.getMaxMessageSizeExponent(message) == 12) {
            socket.add(SocketHelper.getError(SocketHelper.ERROR_SERIALIZER_NOT_SUPPORTED));
          }
        });
      });

      // error 1
      SocketTransport transport = SocketTransport("127.0.0.1", 9003, new Serializer(), SocketHelper.SERIALIZATION_JSON, messageLengthExponent: 9);
      await transport.open();
      ...
      // error 2
      transport = SocketTransport("127.0.0.1", 9003, new Serializer(), SocketHelper.SERIALIZATION_JSON, messageLengthExponent: 10);
      await transport.open();
      errorCompleter = new Completer();
      transport.onOpen.then((aVoid) {}, onError: (error) => errorCompleter.complete(error));
      transport.receive().listen(
        (message) {},
        onError: (error) => transport.onDisconnect.complete(error),
      ...
      expect(error["error"], isNotNull);
      expect(error["errorNumber"], equals(SocketHelper.ERROR_USE_OF_RESERVED_BITS));
      await transport.onDisconnect.future;
      expect(transport.isOpen, isFalse);

      // error 3
      transport = SocketTransport("127.0.0.1", 9003, new Serializer(), SocketHelper.SERIALIZATION_JSON, messageLengthExponent: 11);
      await transport.open();
      errorCompleter = new Completer();
      transport.onOpen.then((aVoid) {}, onError: (error) => errorCompleter.complete(error));
      transport.receive().listen(
      ...
      error = await errorCompleter.future;
      expect(error["error"], isNotNull);
      expect(error["errorNumber"], equals(SocketHelper.ERROR_MESSAGE_LENGTH_EXCEEDED));
      await transport.onDisconnect.future;
      expect(transport.isOpen, isFalse);

      // error 4
      transport = SocketTransport("127.0.0.1", 9003, new Serializer(), SocketHelper.SERIALIZATION_JSON, messageLengthExponent: 12);
      await transport.open();
      errorCompleter = new Completer();
      transport.onOpen.then((aVoid) {}, onError: (error) => errorCompleter.complete(error));
      transport.receive().listen(
      ...
      error = await errorCompleter.future;
      expect(error["error"], isNotNull);
      expect(error["errorNumber"], equals(SocketHelper.ERROR_SERIALIZER_NOT_SUPPORTED));
      await transport.onDisconnect.future;
      expect(transport.isOpen, isFalse);
    });
    test('Ping Pong', () async {
      Serializer serializer = new Serializer();
      Completer pongCompleter = new Completer();
      final server = await ServerSocket.bind("0.0.0.0", 9004);
      server.listen((socket) {
        socket.listen((message) {
          if (message[0] == 0x7F) {
            socket.add(SocketHelper.getInitialHandshake(SocketHelper.MAX_MESSAGE_LENGTH_EXPONENT, SocketHelper.SERIALIZATION_JSON));
          }
          if (message[0] == SocketHelper.MESSAGE_PING) {
            socket.add(SocketHelper.getPong(0, false)+SocketHelper.getPing(false));
          }
      ...
      final transport = SocketTransport("127.0.0.1", 9004, serializer, SocketHelper.SERIALIZATION_JSON, messageLengthExponent: SocketHelper.MAX_MESSAGE_LENGTH_EXPONENT);
      await transport.open();
      transport.receive().listen((message) {});
