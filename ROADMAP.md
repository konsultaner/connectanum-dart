...
- [ ] HTTP bridge (general-purpose request handling)
  ...
  - [x] Support per-method HTTP route action overrides in Dart config, settings codec, native config encoding, and synthetic route matching so a path/protocol match can dispatch individual methods to explicit WAMP realms/procedures or reserved/namespace targets.
  ...
  - [x] Add first route-level rate-limit guard for HTTP route actions (`rate_limit` / `rateLimit`), enforced before auth, MCP, handler, publish, or RPC dispatch with structured 429 responses and Streamable HTTP CORS headers preserved on MCP preflight paths. Caller-scoped bucket state is now capped per stable route/method action, reclaims expired entries, digests bearer/header keys, and fails closed without leaking authorization or MCP session/resume headers through request telemetry.
...
- [ ] HTTP authentication & session tokens
  ...
  - [x] Add config-driven HTTP bearer auth providers (`http_auth_providers`) on top of shared `session_profiles`, so protected HTTP routes can validate JWT/OIDC bearer tokens locally or OAuth access tokens through introspection and then map the result into the same internal auth context used by the WAMP-backed challenge bridge. OAuth introspection now uses one total connection/header/body deadline, bounded response consumption, and redacted fail-closed timeout/oversize/malformed-response results.
  ...
  - [x] Bound successful HTTP-auth grant lineages per realm with `max_http_auth_grants`, counting each refresh-backed or access-only lineage once; new authentication fails closed before challenge allocation and after challenge completion, while refresh rotation remains available and revocation/expiry release capacity.
...
- [x] Call cancellation modes (`skip`, `kill`, `killnowait`) — ensure cancellers can wait for cleanup so subsequent processing shuts down gracefully, and reject unsupported modes instead of silently downgrading them.
  - [x] `killnowait`
  - [x] `kill`
...
- [ ] MCP (Model Context Protocol) server implementation for agentic AI integrations
  ...
  - [x] Make terminal client close reject new and cancel in-flight ordinary MCP,
    OAuth, and router HTTP-auth operations, including requests blocked on
