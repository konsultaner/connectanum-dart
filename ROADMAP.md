...
- [ ] HTTP bridge (general-purpose request handling)
  ...
  - [ ] Introduce adapter pipeline support (static file handler, PHP-FPM/FastCGI bridge, reverse proxy stubs) configurable per route; document adapter contracts and lifecycle.
    ...
    - [x] Reverse proxy and FastCGI route actions are accepted as configuration-visible stubs: aliases parse and round-trip through settings, native routing enqueues them through the HTTP bridge, and the Dart binding returns structured `501 Not Implemented` responses without WAMP dispatch until the real adapters land.
