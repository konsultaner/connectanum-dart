...
- [ ] HTTP bridge (general-purpose request handling)
  ...
  - [ ] Introduce adapter pipeline support (static file handler, reverse proxy, PHP-FPM/FastCGI bridge) configurable per route; document adapter contracts and lifecycle.
    ...
    - [x] FastCGI route actions are accepted as configuration-visible stubs: aliases parse and round-trip through settings, native routing enqueues them through the HTTP bridge, and the Dart binding returns structured `501 Not Implemented` responses without WAMP dispatch until the real adapter lands.
