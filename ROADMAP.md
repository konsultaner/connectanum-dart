...
- [ ] Benchmarks (throughput/latency per worker configuration)
  ...
  - [x] Add serializer-aware WAMP bench scenarios (`serializer = json|msgpack|cbor`), keep representative serializer coverage in `wamp_smoke.toml` / `all_transports_smoke.toml`, and ship a dedicated `wamp_serializer_matrix.toml` for larger-payload RawSocket/WebSocket RPC sweeps.
