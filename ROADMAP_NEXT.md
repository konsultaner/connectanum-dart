...
8. **Benchmark Readiness**
  ...
  - Next: extend the bench to compare Dart versus native client transports on the same WAMP workloads, then move remaining client-side frame/message/serializer overhead out of the Dart session path where the native transport still hands large work back to Dart. Keep release-built thread sweeps as the default for throughput claims, and keep pushing on the HTTP/3 vs HTTP/2 gap in `all_transports_smoke`.
