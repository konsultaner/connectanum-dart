...
- HTTP/3 now applies explicit Quinn transport tuning on both the router and the Rust bench client (stream windows, send window, datagram buffers, keep-alive) instead of pure library defaults; release-built H3-only sweeps in this environment now hold roughly 3.9 Gbps at 1 native runtime thread and 4.6 Gbps at 2 threads, with the earlier 6-thread collapse gone. The shipped `h3_multiplex_scaling.toml` scenario now sweeps `streams_per_connection = 1, 2, 4, 8, 16`, and the latest local Darwin direction sweep across `router_workers = 1,4` plus `native_runtime_threads = 1,4` shows the next H3 follow-up should target transport/backpressure tuning, not application response scheduling. The best low-depth point was `721.60 Mbps` / p95 `54.61 ms` at `s1` with `threads=1, workers=4`, the best overall point was `761.52 Mbps` / p95 `124.85 ms` at `s2` with `threads=4, workers=1`, and the deeper `s8/s16` cases still tracked `82-117` backpressure events instead of a clean worker-scaling win.
...
1. **Boss Telemetry Stream & Prometheus Exporter**
   ...
   - ✅ Metrics snapshot exporter metadata redacts configured OpenMetrics bearer tokens and exposes only a non-secret `auth_required` flag.
...
7. **Remote Authentication Hardening & HTTP Auth Bridge**
   ...
   - ✅ Add a dedicated HTTP bearer-provider bench (JWT local validation and OAuth introspection) so the new provider-backed route path is measured separately from the ticket bridge smoke. `http_bearer_provider_smoke.toml` now covers `/bench/secure-jwt` and `/bench/secure-oauth`, and the Dart bench runner hosts the local introspection endpoint needed by the shipped `oauth` provider config. Introspection response handling is bounded by one total deadline and an explicit size cap, with redacted fail-closed results for stalled, oversized, malformed, or interrupted upstream responses.
...
8. **Benchmark Readiness**
  ...
  - ✅ The bench now has a comparable cross-transport/auth/authz smoke matrix too: WAMP workloads accept explicit realm/auth credentials, can measure connect-only auth via `mode = "authenticate"`, and `transport_mbit_matrix_smoke.toml` reports RawSocket/WebSocket RPC + pub/sub across ACL-off (`bench.control`) and ACL-on (`bench.secure`) realms alongside HTTP/1.1/2/3 public and authenticated routes in one artifact set.
  - ✅ Remote-auth latency now has an explicit first-hit versus warmed-path bench split too: `remote_auth_rawsocket_cold_warm.toml` runs one cold `wamp_rawsocket_auth_frames` ticket login before a warmed follow-up sweep so the router-to-auth-service session setup cost does not get hidden inside one mixed average.
...
12. **Packaging & Build Hooks**
  ...
  - ✅ Both hooks now read cache-safe `hooks.user_defines` for `CONNECTANUM_NATIVE_LIB` prebuilt binaries and `CONNECTANUM_SKIP_NATIVE_BUILD` system/shared-library deployments instead of relying on Dart SDK-stripped shell environment variables or invoking Cargo unconditionally.
  - ✅ A dedicated GitHub Actions workflow now packages explicit Linux x64, Linux arm64, macOS arm64, and macOS Intel `ct_ffi` archives through `bin/package-native-artifact`, uploading reusable bundles for the existing `CONNECTANUM_NATIVE_LIB` contract.
  - ✅ Release-tag and explicit manual native-artifact runs are now configured to publish those packaged bundles to GitHub Releases, so the `CONNECTANUM_NATIVE_LIB` path has a durable hosted download target instead of workflow-artifact retention only.
  - ✅ The router/client hooks can now download and checksum-verify hosted `ct_ffi` release bundles directly when `CONNECTANUM_NATIVE_RELEASE_TAG` is set as a hook user define, so downstream users can opt into prebuilt binaries without Cargo or manual extraction.
  - ✅ `packages/connectanum_router/tool/install_native.dart` and `packages/connectanum_client/tool/install_native.dart` now provide an explicit source-checkout prefetch path: they download and checksum-verify the hosted bundle into `.dart_tool/connectanum/native/<host-triple>/` and print the resulting library path for `CONNECTANUM_NATIVE_LIB`.
