...
- WAMP bench coverage now spans both RawSocket and WebSocket: the bench runner resolves explicit WAMP transport targets from `bench_router.json`, `wamp_smoke.toml` compares both transports directly, `all_transports_smoke.toml` exercises RawSocket/WebSocket/HTTP/1.1/HTTP/2/HTTP/3 in one run, and WAMP workloads now accept `serializer = json|msgpack|cbor` so the shipped scenarios can benchmark serializer overhead directly. The pubsub matcher now uses a persistent event buffer, so the bench no longer drops a fast EVENT between iterations and hang-spins for 30 seconds.
...
- A fresh release-built `all_transports_smoke` run on this laptop now lands roughly at `6.8 ms` RawSocket pub/sub, `4.8 ms` RawSocket RPC, `3.5 ms` WebSocket pub/sub, `4.8 ms` WebSocket RPC, `178 Mbps` HTTP/1.1 response throughput, `559 Mbps` HTTP/2, and `365 Mbps` HTTP/3. The remaining transport gap is HTTP/3 staying materially behind HTTP/2 on the same bench shape.
...
8. **Benchmark Readiness**
   ...
   - Next: add CI gating over the transformed artifacts, keep release-built thread sweeps as the default for throughput claims, extend the WAMP side with larger-payload pub/sub sweeps so RawSocket vs WebSocket ceilings can be mapped separately from the HTTP transport ceilings, and investigate why HTTP/3 still trails HTTP/2 on the shared `all_transports_smoke` workload after the transport-tuning fixes.
